...
function createTetrisField ( parent )
	...
	music = playSound ( "sound/theme.ogg", true )
...
function startGame()
	...
	gameTimer = setTimer ( startBlock, 2500, 1 )
	for y = 18, 1, -1 do
		for x = 10, 1, -1 do
...
function rowFinished ( row )
	...
	for i = 1, 10 do
		setTimer ( removeBlock, 50, 1, i, row )
		fixedFields[i][row] = false
	end
	setTimer ( moveDownRows, 250, 1, row )
end
